...
def parse_args():
    ...
    ap.add_argument('--lbaas-agent-check', action='store_true', default=False,
                    help='Show LBaaS associated with offline lbaas agents')
    ap.add_argument('--l3-agent-migrate', action='store_true', default=False,
                    help='Migrate routers away from offline l3 agents')
    ap.add_argument('--lbaas-agent-migrate', action='store_true', default=False,
                    help='Migrate LBaaS pools away from offline lbaas agents')
...
def run(args):
    ...
    if args.lbaas_agent_check:
        LOG.info("Performing LBaaS Agent Health Check")
        lbaas_agent_check(qclient)
    ...
    if args.lbaas_agent_migrate:
        LOG.info("Performing LBaaS Agent Pool Migration for Offline LBaaS Agents")
        lbaas_agent_migrate(qclient, args.noop, args.now)
...
def lbaas_agent_check(qclient):
    """
    Walk the lbaas agents searching for agents that are offline.  Show pools
    that are offline and where we would migrate them to.
    ...
    migration_count = 0
    agent_list = list_agents(qclient)
    agent_dead_list = agent_dead_id_list(agent_list, 'Loadbalancer agent')
    agent_alive_list = agent_alive_id_list(agent_list, 'Loadbalancer agent')
    LOG.info("There are %s offline LBaaS agents and %s online LBaaS agents",
             len(agent_dead_list), len(agent_alive_list))

    if len(agent_dead_list) > 0:
        for agent_id in agent_dead_list:
            LOG.info("Querying agent_id=%s for lbaas to migrate", agent_id)
            pools_id_list = list_pools_on_lbaas_agent(qclient, agent_id)

            for pool_id in pools_id_list:
                try:
                    target_id = random.choice(agent_alive_list)
                except IndexError:
                    LOG.warn("There are no LBaaS agents alive we could migrate routers onto.")
                    target_id = None

                migration_count += 1
                LOG.warn("Would like to migrate LBaaS pool=%s", pool_id)

        if migration_count > 0:
            sys.exit(2)

def lbaas_agent_migrate(qclient, noop=False, now=False):
    """
    Walk the lbaas agents searching for agents that are offline.  Show pools
    that are offline and migrate them.
    ...
    agent_dead_list = agent_dead_id_list(agent_list, 'Loadbalancer agent')
    agent_alive_list = agent_alive_id_list(agent_list, 'Loadbalancer agent')
    LOG.info("There are %s offline LBaaS agents and %s online LBaaS agents", len(agent_dead_list), len(agent_alive_list))
    ...
    if len(agent_dead_list) > 0:
        for agent_id in agent_dead_list:
            ...
            for pool_id in pools_id_list:
                target_id = random.choice(agent_alive_list)
                LOG.info("Migrating LBaaS pool=%s", pool_id)

                try:
                    if not noop:
                        migrate_lbaas_pool(qclient, pool_id, agent_id, target_id, agent_alive_list)
                        migration_count += 1
                except:
                    LOG.exception("There was an error migrating a LBaaS pool")
                    continue

        LOG.info("%s LBaaS pools were migrated from offline LBaaS agents", migration_count)
...
def migrate_lbaas_pool(qclient, pool_id, agent_id, target_id, agent_alive_list):
    """
    Returns nothing, and raises on exception

    :param qclient: A neutronclient
    :param pool_id: The id of the lbaas pool to migrate
    :param agent_id: The id of the lbaas agent to migrate from
    :param target_id: The id of the lbaas agent to migrate to
    :param agent_alive_list: List of possible agents where the migrated pool gets created
    """
    ...
    pool_info = qclient.show_pool(pool_id)
    vip_info = qclient.show_vip(pool_info['pool']['vip_id'])
    floatingip_info = ""
    floating_ips = qclient.list_floatingips()
    for floating_ip in floating_ips['floatingips']:
      if floating_ip['port_id'] == vip_info['vip']['port_id']:
        floatingip_info = qclient.show_floatingip(floating_ip['id'])
        break
    members_info = [];
    for member in pool_info['pool']['members']:
      member_info = qclient.show_member(member)
      members_info.append(member_info['member'])
    LOG.info("Trying to recreate LBaaS pool %s", pool_info['pool']['name'])

    # Delete LBaaS pool
    for health_monitor in pool_info['pool']['health_monitors']:
      qclient.disassociate_health_monitor(pool_id, health_monitor)
    for member in members_info:
      qclient.delete_member(member['id'])
    qclient.delete_vip(pool_info['pool']['vip_id'])
    qclient.delete_pool(pool_id)

    # ensure it is removed or log an error
    if pool_id in list_pools_on_lbaas_agent(qclient, agent_id):
      LOG.exception("Failed to remove router_id=%s from agent_id=%s", pool_id, agent_id)

    # recreate pool, too create it on an alive lbaas agent
    pool_body = {'pool': {'name': pool_info['pool']['name'],
                 'tenant_id': pool_info['pool']['tenant_id'],
                 'subnet_id': pool_info['pool']['subnet_id'],
                 'protocol': pool_info['pool']['protocol'],
                 'lb_method': pool_info['pool']['lb_method']
                }}
    pool_info_new = qclient.create_pool(pool_body)

    new_pool_id = pool_info_new['pool']['id']
    vip_body = {'vip': { 'name': vip_info['vip']['name'],
                'protocol': vip_info['vip']['protocol'],
                'subnet_id': vip_info['vip']['subnet_id'],
                'pool_id': new_pool_id,
                'address': vip_info['vip']['address'],
                'tenant_id': vip_info['vip']['tenant_id'],
                'protocol_port': vip_info['vip']['protocol_port']
               }}
    new_vip = qclient.create_vip(vip_body)
    for member in members_info:
      member_body = {'member': { 'address': member['address'],
                     'protocol_port': member['protocol_port'],
                     'weight': member['weight'],
                     'tenant_id': member['tenant_id'],
                     'pool_id': new_pool_id
                    }}
      member_info = qclient.create_member(member_body)
    # associate healthmonitors
    health_monitors = []
    for health_monitor_id in pool_info['pool']['health_monitors']:
      qclient.associate_health_monitor(new_pool_id, {'health_monitor': {'id': health_monitor_id }})
    # associate new vip port_id to floating ip
    qclient.update_floatingip(floatingip_info['floatingip']['id'], {'floatingip': {'port_id': new_vip['vip']['port_id']}})

    if new_pool_id:
      LOG.info("Recreate LBaaS pool %s with id %s", pool_info['pool']['name'], new_pool_id)
    else:
      LOG.exception("Failed to create pool %s", pool_info['pool']['name'])
...
def list_pools_on_lbaas_agent(qclient, agent_id):
    """
    Return a list of router ids on an agent
    ...
    resp = qclient.list_pools_on_lbaas_agent(agent_id)
    LOG.debug("list_pools_on_lbaas_agent: %s", resp)
    return [r['id'] for r in resp['pools']]
